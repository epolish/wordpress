...
function filter_post_data( $data , $post_arr ) {
    ...
    if (
        $data['post_status'] != 'auto-draft' && $data['post_status'] != 'trash'
        && $founded_post && $founded_post->ID != $post_arr['post_ID']
        && $founded_post->post_status != 'trash'
    ) {
...
function api_discount() {
    ...
    if ( $post_id ) {
        $post = get_page_by_title( $post_id, OBJECT, POST_TYPE_ERST_DISCOUNT );
        if ( $post && $post->post_type == POST_TYPE_ERST_DISCOUNT ) {
            $discount_size = get_post_meta( $post->ID )['erst_discount_size'][0];
            if ($discount_size) {
                $response['status'] = 200;
...
/**
 * Return discount size by number
 *
 * @param	string $number
 * @return	float|null
 */
function get_discount_size_by_number($number) {
    $page = get_page_by_title( $number, OBJECT, POST_TYPE_ERST_DISCOUNT );
    if ( !$page ) {
        return null;
    }
    return floatval( get_post_meta( $page->ID, 'erst_discount_size' , true ) );
}